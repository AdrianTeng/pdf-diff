...
def compute_changes(pdf_fn_1, pdf_fn_2, top_margin=0, bottom_margin=100):
    ...
    false_true_dict = dict()
    for item in changes:
        if item != '*':
            # add x to filter as we may delete same texts which are in different location if using only the text, 
            # seems like x will be equal if false highlight occurs
            if '{},{}'.format(item['x'], item['text']) in false_true_dict:
                false_true_dict['{},{}'.format(item['x'], item['text'])] = True # appear again, set to True
            else:
                false_true_dict['{},{}'.format(item['x'], item['text'])] = False
    for key, value in false_true_dict.items():
        if value:
            is_loop_finish = False
            while not is_loop_finish:
                is_sth_removed = False
                for item in changes:
                    if item != '*':
                        keys = key.split(',')
                        if str(item['x']) == keys[0] and str(item['text']) == keys[1]:
                            #sys.stderr.write('{} {}\n'.format(keys[0], keys[1]))
                            changes.remove(item)
                            is_sth_removed = True
                if not is_sth_removed:
                    is_loop_finish = True
    return changes
...
def render_changes(changes, styles,width, file1, file2):
    ...
    pages = make_pages_images(changes,width, [file1, file2])
...
def make_pages_images(changes,width, files):
    pages = [{}, {}]
    for index in (0,1):
        pdf = PdfFileReader(open(files[index],'rb'))
        for page in range(pdf.getNumPages()):
            pages[index][page+1] = pdftopng(files[index], page+1,width)
    return pages
...
def main():
    ...
    img = render_changes(changes, style, args.result_width, args.files[0], args.files[1])
    img.save(sys.stdout.buffer, args.format.upper())
